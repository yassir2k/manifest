<?php
	...
	$batch_code = $_POST['reg-batch-code'];
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								$sql.= "WHERE REG_Batch = '$batch_code' ";
								$result = sqlsrv_query($conn, $sql);
								if($result === false) 
								{
								...
								else
								{
									...
									$sql.= " State_Code) ";
									$sql.= "SELECT Serial_No, RC_Number, Company_Name, Company_Type,'$courier','$user',GETDATE(), ";
									$sql.= " '$batch_code','$mr_code','$scode' FROM tbl_Mail_Room WHERE REG_Batch = '$batch_code'";
									$result = sqlsrv_query($conn, $sql);
									if($result === false) 
									{

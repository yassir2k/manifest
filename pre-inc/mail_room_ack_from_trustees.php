...
<body>
  ...
  <ol class="breadcrumb" style="background-color: #F5F5F5; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px">
				...
				<li class="breadcrumb-item active text-secondary">From Incorporated Trustees</li>
  </ol>
   ...
   <div class="row top-buffer">
			...
			<div class="table-wrapper">
				<div style="padding-bottom: 15px; background: #666; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 28px"><b>Mail Room Manifest Acknowledgement</b> (From Trustees - Pre Inc.)</h2>
						</div>
						<div class="col-xs-6 ml-auto">
							...
							<button id="showITModal" disabled="disabled" class="btn btn-info" data-toggle="modal"><i class="fa fa-check-circle"></i> <span>Confirm Acknowledgements</span></button>						
						</div>
					</div>
				</div>
				<table class="table table-striped table-hover" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
					<thead>
						<tr id="">
							...
							<th>Serial Number</th>
							<th>RC Number</th>
							<th>Actions (Ack. Manifest From Trustees)</th>
						</tr>
					</thead>
					<?php
					//Now we read all forwarded manifest from Incorporated Trustees to Mail Room
					$query = "SELECT Company_Name, Serial_No, RC_Number FROM tbl_Trustees WHERE ";
					$query.="(IT_Despatch_Status = 'S-T-MR')";
	...
	<!-- Acknowledge from RGs Office Modal (Prompt) -->
		<div id="ackFromITModal" class="modal fade" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif"> 
		<div class="modal-dialog">
			<div class="modal-content">
					<div class="modal-header">						
						<h4 class="modal-title">Acknowledge from Incorporated Trustees</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					</div>
					<div class="modal-body"> 		
						<p>Are you sure you want to acknowledge the receipt of this manifest from IT?</p>
...
function checkOnorOff()
{
	...
	checkbox.each(function(){
		...
		if(check_counter > 0)
			$("#showITModal").prop('disabled', false);
		else
			$("#showITModal").prop('disabled', true);

	});
}

//This shows Modal
$("#showITModal").click(function(){
	$("#bc").html('<label style="font-weight: bold; color: red">' + check_counter + '</label>');
	$("#ackFromITModal").modal("show");
});

//This handles event when modal is closed
$('#ackFromITModal').on('hidden.bs.modal', function () {

	if(flag == 1){
		setTimeout(() => { window.location.assign("mail_room_ack_from_trustees.php"); });
	}
})
...
$("#confirmAck").click(function(){
	...
	$.ajax({  
		url:"mail_room_bg_ack_from_trustees.php",  
		method:"POST",  
		data:d,  

...
<head>
  ...
  <title>E - Despatch: Business Names Dashboard</title>
</head>
...
<div class="container-bg">
  <div class="fcta-header">
   ...
   <?php require 'menus/menu_bn_dashboard.php';?>
   <!-- ---------------------------------------End of Navigation------------------------------------------- -->
   ...
   <div class="row top-buffer">
        ...
        <div class="col-sm-8"> <!--Main Row Left-->
			...
			<?php 
				//Total No of Certificates Generated BN --> Mail Room
				$sql = "SELECT count(*) as Total FROM tbl_Business_Names where convert(varchar(10), ";
				$sql.= "BN_Manifest_Preparation_Date, 102) = convert(varchar(10), getdate(), 102) AND (BN_Despatch_Status = 'S-T-MR' ";
				...
				$sql = "SELECT count(*) as Total FROM tbl_Business_Names where convert(varchar(10), ";
				$sql.= "BN_Manifest_Preparation_Date, 102) = convert(varchar(10), getdate(), 102) AND ( ";
				$sql.= " BN_Despatch_Status = 'A-B-MR') AND State_Code='$scode'";
				$result = sqlsrv_query($conn, $sql);
				if($result === false) {
					die(print_r(sqlsrv_errors(), true));
				}
			...
			<div class="row"><!--Beginning of Putting Up Stats For Registry-->
				...
				$sql = "SELECT count(*) as Total FROM tbl_Business_Names where (BN_Despatch_Status = 'S-T-MR' ";
				$sql.= "OR BN_Despatch_Status = 'A-B-MR') AND State_Code='$scode'";
				$result = sqlsrv_query($conn, $sql);
				if($result === false) {
					die(print_r(sqlsrv_errors(), true));
				...
				//Total Acknowledged by Mail Room
				$sql = "SELECT count(*) as Total FROM tbl_Business_Names where ( ";
				$sql.= " BN_Despatch_Status = 'A-B-MR') AND State_Code='$scode'";
				$result = sqlsrv_query($conn, $sql);
				if($result === false) {
					die(print_r(sqlsrv_errors(), true));
			...
			<div class="row top-space20" ><!--Manifest Statistics IT-->
                ...
                <div class="col-sm-6" align="center">
                    <h6 class="text-success" align="center"><b>Total Manifest Statistics (Mail Room)</b></h6>
                </div>
                <div class="col-sm-3" align="center">
                    <hr class="separator">
                </div>

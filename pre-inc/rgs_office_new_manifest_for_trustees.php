...
$sql = "SELECT TOP 1 Batch_Number FROM tbl_Batches_History WHERE Department_Unit = 'RGs Office' ";
...
<head>
  ...
  <title>E - Manifest: Manifest Entry (Incorporated Trustees)</title>
</head>
...
<body>
  ...
  <!--Add Menu Here-->
  <?php require('menus/menu_rgs_office_new_manifest.php'); ?>
  <!--End of Menu-->
  <ol class="breadcrumb" style="background-color: #F5F5F5; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px">
				<li class="breadcrumb-item"><a class="text-success" href="rgs_office_dashboard.php">Dashboard</a></li>
				<li class="breadcrumb-item active text-secondary">File a New Manifest</li>
				<li class="breadcrumb-item active text-secondary">Incorporated Trustees</li>
  </ol>
<div class="toptop" >
   <div class="row top-buffer">
			...
			<div class="table-wrapper">
				<div class="" style="padding-bottom: 15px; background: #777; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 30px"><b>Manifest Entry For Incorporated Trustees (Pre Incorporation)</b></h2>
						</div>
					</div>
				</div>
					...
					<div class="col-sm-6">
						<div class="form-group">
							<button disabled id="submitR" type="submit" class="btn btn-success btn-block"><i class="fa fa-send"></i>
								<span style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif">Forward to Incorporated Trustees</span>
							</button>
						</div>
					</div>
					<div class="col-sm-3">
	...
	<br />
	<br />
<!-- Forward to Trustees Modal (Prompt) -->
	<div id="toTrusteesModal" class="modal fade" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif"> 
		<div class="modal-dialog">
			<div class="modal-content">
					<div class="modal-header">						
						<h4 class="modal-title">Forward to Incorporated Trustees</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					</div>
					<div class="modal-body"> 		
						<p>Are you sure you want to forward this manifest to Incorporated Trustees? </p>
					...
					<div class="modal-footer">
						...
						<span>Yes, Forward to Incorporated Trustees</span>
							<i class="fa fa-check" style="horizontal-align: right;" ></i>
						</button>
					</div>
			</div>
		</div>
        </div><!--End of Trustees Modal-->
  ...
  .change(function () {
	  $( "#tblS" ).empty() //Clear any previously generated table
		...
		var table = $('<table  id="mainTable" width="100%" class="table-bordered table-sm" style="font-family:"Trebuchet MS", Arial, Helvetica, sans-serif; font-size:16px"><thead style="background-color:#EEE"><tr><th style="width:5%">NO.</th><th style="width:55%">COMPANY NAME</th><th style="width:20%">SERIAL NUMBER</th><th style="width:20%">RC NUMBER</th></tr></thead><tbody>');
		...
		for(var r = 0; r < rows; r++)
		{
			...
			else
			{
				...
				if(r != 0)
					tr = $('<tr>');
				$('<th style="background-color:#EEE">'+(r+1)+'</th><td></td><td></td><td></td></tr>').appendTo(tr); //fill in your cells with something meaningful here
				tr.appendTo(table);
			}
		}
...
$("#submitR").click(function(){
	...
	$("#toTrusteesModal").modal("show");
});

//This handles event when modal is closed

$('#toTrusteesModal').on('hidden.bs.modal', function () {

	if(flag == 1){
		setTimeout(() => { window.location.assign("rgs_office_new_manifest_for_trustees.php"); });
	}
})
...
$("#confirmFwd").click(function(){
	if(!$('#fDate')[0].checkValidity()) {
	  ...
	  $.post("rgs_office_new_manifest_for_trustees.php");
	  return;
	}
  var table = document.getElementById('mainTable');
  var companies = [];
  ...
  var index = $("#indexer").val();
   var batch = "<?php echo $_SESSION['state_code'].'-RGO-'.date('dmYHis').'-PRE-B-'; ?>";
   batch = batch.concat(index);
  //First, We Check For Empty Fields in all the spreadsheet cells.
  for (var r = 1; r <= table.rows.length - 1; r++) {        // EACH ROW IN THE TABLE Except the Header.
            for (c = 1; c <= table.rows[r].cells.length - 1; c++) {      // EACH CELL IN A ROW Except the leftmost ones.
                 if(table.rows[r].cells[c].innerHTML.replace(/^\s+|\s+$/g, "").length == 0)
				 {
					 ...
					 if(c == 2)
						 serials.push(table.rows[r].cells[c].innerHTML);
					 if(c == 3){
  ...
  var d = 'data={"companies":["' + companies.join('","') + '"],"serials":["' + serials.join('","') + '"],"rc_numbers":["' + rc_numbers.join('","') + '"],"cert_date":["' + cert_date.join('","') + '"],"cert_batch":["' + cert_batch.join('","') + '"]}';
	$.ajax({  
                url:"rgs_office_bg_to_trustees.php",  
                method:"POST",  
                data:d,  

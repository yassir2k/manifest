...
<head>
  ...
  <title>Manifest Entry (Registry)</title>
</head>
...
<body>
  ...
  <ol class="breadcrumb" style="background-color: #F5F5F5; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px">
				...
				<li class="breadcrumb-item active text-secondary">Registry</li>
  </ol>
<div class="toptop" >
			...
			<div class="table-wrapper">
				<div class="" style="padding-bottom: 15px; background: #777; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 30px"><b>Manifest Entry For Registry (Pre Incorporation)</b></h2>
						</div>
					</div>
				</div>
					...
					<div class="col-sm-6">
						<div class="form-group">
							<button disabled id="submitR" type="submit" class="btn btn-success btn-block"><i class="fa fa-send"></i>
								<span style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif">Forward to Registry</span>
							</button>
						</div>
					</div>
					<div class="col-sm-3">
...
<!-- Forward to Registry Modal (Prompt) -->
	<div id="toRegistryModal" class="modal fade" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif"> 
		<div class="modal-dialog">
			<div class="modal-content">
					<div class="modal-header">						
						<h4 class="modal-title">Forward to Registry</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					</div>
					<div class="modal-body"> 		
						<p>Are you sure you want to forward this manifest to Registry? </p>
					...
					<div class="modal-footer">
						...
						<span>Yes, Forward to Registry</span>
...
$("#submitR").click(function(){
	...
	$("#toRegistryModal").modal("show");
});

//This handles event when modal is closed

$('#toRegistryModal').on('hidden.bs.modal', function () {

	if(flag == 1){
		setTimeout(() => { window.location.assign("rgs_office_new_manifest_for_registry.php"); });
	}
})
...
$("#confirmFwd").click(function(){
	if(!$('#fDate')[0].checkValidity()) {
	  ...
	  $.post("rgs_office_new_manifest_for_registry.php");
	  return;
	}
  var table = document.getElementById('mainTable');
  var companies = [];
  ...
  var d = 'data={"companies":["' + companies.join('","') + '"],"serials":["' + serials.join('","') + '"],"rc_numbers":["' + rc_numbers.join('","') + '"],"cert_date":["' + cert_date.join('","') + '"],"cert_batch":["' + cert_batch.join('","') + '"]}';
	$.ajax({  
                url:"rgs_office_bg_to_registry.php",  
                method:"POST",  
                data:d,  

...
for ($i = 0; $i < $count; $i++){
	...
	$sql.=",'".$data->companies[$i]."','LLC', 'PRE', '".$data->cert_batch[$i]."',convert(varchar(10), '".$data->cert_date[$i]."', 102), ";
	$sql.="GETDATE(),'S-T-R','$user', '$scode')";
	$result = sqlsrv_query($conn, $sql);
	if($result === false) {
...
$bc = $data->cert_batch[0]; //E.g. FCT-15072020005344-RGO-PRE-BATCH-1
$substring ='-';
...
$event = "New Pre Incorporation Manifest RG -> Registry";
//Insert Into DB
...
sqlsrv_close( $conn );
echo "Certificates Successfully Forwarded to Registry. <br />Batch Code: <strong>$bc</strong><br />";

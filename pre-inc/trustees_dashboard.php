...
|| ( ($_SESSION['dept'] != "Trustees") || ($_SESSION['role'] != "Pre Incorporation") ))
{
...
<head>
  ...
  <title>E - Manifest: IT Dashboard</title>
</head>
...
<div class="container-bg">
  <div class="fcta-header">
   ...
   <?php require 'menus/menu_it_dashboard.php';?>
   <!-- ---------------------------------------End of Navigation------------------------------------------- -->
   ...
   <div class="row top-buffer">
        ...
        <div class="col-sm-8"> <!--Main Row Left-->
			...
			<div class="row"><!--Beginning of Putting Up Stats For IT-->
        	...
        	</div><!--End of Putting Up Stats For IT-->
			
				<?php 
				//Total Forwarded (Mail Room)
				$sql = "SELECT count(*) as Total FROM tbl_Trustees where convert(varchar(10), ";
				$sql.= "IT_Manifest_Preparation_Date, 102) = convert(varchar(10), getdate(), 102) AND (IT_Despatch_Status = 'S-T-MR' ";
				$sql.= "OR IT_Despatch_Status = 'A-B-MR') AND State_Code='$scode'";
				$result = sqlsrv_query($conn, $sql);
				if($result === false) {
					die(print_r(sqlsrv_errors(), true));
				}
				...
				$sql = "SELECT count(*) as Total FROM tbl_Trustees where convert(varchar(10), ";
				$sql.= "IT_Manifest_Preparation_Date, 102) = convert(varchar(10), getdate(), 102) AND (IT_Despatch_Status = 'A-B-MR') AND State_Code='$scode'";
				$result = sqlsrv_query($conn, $sql);
				if($result === false) {
					die(print_r(sqlsrv_errors(), true));
				}

...
$(document).ready(function(){
  ...
  <title>E- Despatch: Acknowledge From Incorporated Trustees</title>
</head>
...
<body>
  ...
  <ol class="breadcrumb" style="background-color: #F5F5F5; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px">
				...
				<li class="breadcrumb-item active text-secondary">From Business Names</li>
  </ol>
   ...
   <div class="row top-buffer">
			...
			<div class="table-wrapper">
				<div style="padding-bottom: 15px; background: #666; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 28px"><b>Mail Room Manifest Acknowledgement</b> (From BN - Pre Inc.)</h2>
						</div>
						<div class="col-xs-6 ml-auto">
							...
							<button id="showBNModal" disabled="disabled" class="btn btn-info" data-toggle="modal"><i class="fa fa-check-circle"></i> <span>Confirm Acknowledgements</span></button>						
						</div>
					</div>
				</div>
				<table class="table table-striped table-hover" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
					<thead>
						<tr id="">
							...
							<th>Name</th>
							<th>RC Number</th>
							<th>Actions (Ack. Manifest From Business Names)</th>
						</tr>
					</thead>
					<?php
					//Now we read all forwarded manifest from Incorporated Trustees to Mail Room
					$query = "SELECT Business_Name, BN_Number FROM tbl_Business_Names WHERE ";
					$query.="(BN_Despatch_Status = 'S-T-MR')";
					...
					<tbody>
						<tr>
							<td colspan="5">
								No new manifest yet from the Business Names Department.
							</td>
						</tr>
					</tbody>
					<?php
					...
					else
					{
						...
						{
					?>
						<tr style="vertical-align:middle" class="<?php echo $row['BN_Number']; ?>">
							<td><?php echo $i+1; ?></td>
	...
	<!-- Acknowledge from RGs Office Modal (Prompt) -->
		<div id="ackFromBNModal" class="modal fade" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif"> 
		<div class="modal-dialog">
			<div class="modal-content">
					<div class="modal-header">						
						<h4 class="modal-title">Acknowledge from Business Names</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					</div>
					<div class="modal-body"> 		
						<p>Are you sure you want to acknowledge the receipt of this manifest from BN?</p>
...
function checkOnorOff()
{
	...
	checkbox.each(function(){
		...
		if(check_counter > 0)
			$("#showBNModal").prop('disabled', false);
		else
			$("#showBNModal").prop('disabled', true);

	});
}

//This shows Modal
$("#showBNModal").click(function(){
	$("#bc").html('<label style="font-weight: bold; color: red">' + check_counter + '</label>');
	$("#ackFromBNModal").modal("show");
});

//This handles event when modal is closed
$('#ackFromBNModal').on('hidden.bs.modal', function () {

	if(flag == 1){
		setTimeout(() => { window.location.assign("mail_room_ack_from_bn.php"); });
	}
})

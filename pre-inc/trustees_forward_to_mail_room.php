...
$sql = "SELECT TOP 1 Batch_Number FROM tbl_Batches_History WHERE Department_Unit = 'Trustees' ";
...
$(document).ready(function(){
  ...
  <title>E - Manifest: IT Manifest to Mail Room</title>
</head>
...
<body>
  ...
  <!--Add Menu Here-->
  <?php require('menus/menu_it_forward_to_mail_room.php')?>
  <!--End of Menu-->
        ...
        <div class="table-responsive">
			<div class="table-wrapper">
				<div style="padding-bottom: 15px; background: #777; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 24px">Incorporated Trustees's Manifest<b> To Mail Room</b></h2>
						</div>
				...
				<table class="table table-striped table-hover" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
					...
					<?php
					//Now we read all forwarded manifest from RGs office to Trustees
					include ("../dbconnection.php"); 
					$query = "SELECT Company_Name, Serial_No, RC_Number FROM tbl_Trustees WHERE IT_Despatch_Status IS NULL";
...
$('#forwardToMailRoomModal').on('hidden.bs.modal', function () {
	...
	if(flag == 1){
		setTimeout(() => { window.location.assign("trustees_forward_to_mail_room.php"); });
	}
})
...
$("#fwdToMailRoom").click(function(){
	...
	var batch = "<?php echo $_SESSION['state_code'].'-IT-'.date('dmYHis').'-PRE-B-'; ?>";
	...
	$.ajax({  
		url:"trustees_bg_forwarded_to_mail_room.php",  
		method:"POST",  
		data:d,  

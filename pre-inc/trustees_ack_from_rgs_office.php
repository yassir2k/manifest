...
$query = "SELECT Company_Name, Serial_No, RC_Number FROM tbl_RGs_office WHERE (RGO_Despatch_Status = 'S-T-T')";
...
$(document).ready(function(){
  ...
  <title>E - Manifest: Acknowledge From RGs Office</title>
</head>
...
<body>
  ...
  <!--Add Menu Here-->
  <?php require('menus/menu_it_ack_rgs_office.php')?>
  <!--End of Menu-->
  <ol class="breadcrumb" style="background-color: #F5F5F5; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px">
				<li class="breadcrumb-item"><a class="text-success" href="trustees_dashboard.php">Dashboard</a></li>
				<li class="breadcrumb-item active text-secondary">Acknowledge Manifest</li>
				<li class="breadcrumb-item active text-secondary">From RGs Office</li>
  </ol>
			...
			<div class="table-wrapper">
				<div style="padding-bottom: 15px; background: #777; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 30px"><b>Incorporated Trustees's Manifest</b> (From RG's Office)</h2>
						</div>
				...
				<table class="table table-striped table-hover" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
					<thead>
						<tr id="">
							...
							<th>Serial Number</th>
							<th>RC Number</th>
							<th>Actions (Ack. Manifest From RGs Office)</th>
						</tr>
					</thead>
					<?php
					//Now we read all forwarded manifest from RGs office to Registry
					$query = "SELECT Company_Name, Serial_No, RC_Number FROM tbl_RGs_office WHERE ";
					$query.="(RGO_Despatch_Status = 'S-T-T')";
...
$('#acknowledgeAllModal').on('hidden.bs.modal', function () {
	...
	if(flag == 1){
		setTimeout(() => { window.location.assign("trustees_ack_from_rgs_office.php"); });
	}
})
...
$("#confirmAck").click(function(){
	...
	$.ajax({  
		url:"trustees_bg_ack_manifest_from_rg.php",  
		method:"POST",  
		data:d,  

	...
	$sql="";
	for ($i = 0; $i < $count; $i++){
		$sql = "UPDATE tbl_Registry SET REG_Despatch_Status = 'A-B-MR', MR_Received_By = '".$_SESSION['user']."' , ";
	...
	$sql = "INSERT INTO tbl_Mail_Room (Serial_No, RC_Number, Company_Name, Company_Type, REG_Batch, Received_From_REG_On, State_Code) ";
	$sql.= "SELECT Serial_No, RC_Number, Company_Name, 'LLC', REG_Batch, MR_Receiving_Date,'".$_SESSION['state_code']."' FROM tbl_Registry ";
	$sql.= "WHERE tbl_Registry.Serial_No ";
	$sql.= " = ";
	for($i = 0; $i < $count; $i++)
	{
	if($i == $count-1)
	...
	$event = "Acknowledgement of Manifest from Registry -> Mail Room";
	//Insert Into DB

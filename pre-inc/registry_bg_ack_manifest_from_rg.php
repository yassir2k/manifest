...
for ($i = 0; $i < $count; $i++){
	$sql = "UPDATE tbl_RGs_office SET RGO_Despatch_Status = 'A-B-R', REG_Received_By = '$user' , ";
	$sql.=" REG_Receiving_Date = GETDATE() WHERE Serial_No = '".$data->serials[$i]."'";
	$result = sqlsrv_query($conn, $sql);
	if($result === false) {
		die(print_r(sqlsrv_errors(), true));
	} 
}
//Now Populate Registry's Database
$sql = "INSERT INTO tbl_Registry (Serial_No, RC_Number, Company_Name, Company_Type, Received_From_RGO_On, RGO_Batch, State_Code) ";
$sql.= "SELECT Serial_No, RC_Number, Company_Name, 'LLC', REG_Receiving_Date, RGO_Batch, '".$_SESSION['state_code']."' FROM tbl_RGs_office ";
...
$result = sqlsrv_query($conn, $sql);
	...
	$event = "Acknowledgement of Manifest from RG -> Registry";
	//Insert Into DB

			...
			<div class="table-wrapper">
				<div style="padding-bottom: 15px; background: #666; color: #fff; padding: 16px 30px; margin: -20px -25px 10px; border-radius: 3px 3px 0 0;">
					<div class="row" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
						<div class="col-xs-6">
							<h2 style="margin: 5px 0 0; font-size: 28px"><b>Mail Room Despatch</b> (Forward BN Batch to Courier)</h2>
						</div>
					</div>
				</div>
				<table class="table table-striped table-hover" style="font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif">
					...
					$query.="FROM tbl_Business_Names as R INNER JOIN tbl_Batches_History as B ";
					$query.="ON ((R.BN_Batch = B.Batch_Code and R.MR_Receiving_Date IS NOT NULL) ";
					$query.=" AND R.BN_Despatch_Status = 'A-B-MR') INNER JOIN tbl_Mail_Room as M ON ";
					$query.= "(R.BN_Batch = M.BN_Batch AND M.Forwarded_By IS NULL) ORDER  BY Date_Generated";
...
<script>
	...
	$(document).ready(function(){
		...
		//This Script handles the manipulation of expand and collapse (+) and (-)
			$('.fa').on('click',function(event)
			{
				...
				$.ajax({  
				url:"mail_room_bg_load_manifest_content_bn_pre_inc.php",  
				method:"POST",  
				data:dat,  
	...
	$("#confirmFwd").click(function(){
		...
		var d = "bn-batch-code=" + Global_Batch + "&mr-batch-code=" + batch + "&total-records=" + count;
		$.ajax({  
			url:"mail_room_bg_forward_bn_to_courier.php",  
			method:"POST",  
			data:d,  
			success:function(data)  
			{  
	...
	$('#toCourierModal').on('hidden.bs.modal', function () {
		if(flag == 0)
		{
			...
			setTimeout(() => { window.location.assign("mail_room_despatch_bn_to_courier.php"); });
		}
	})

...
for ($i = 0; $i < $count; $i++){
	$sql = "SELECT * FROM tbl_Business_Names WHERE BN_Number = '". $data->rc_numbers[$i]."' OR ";
	$sql.= "Business_Name = '".$data->companies[$i]."' ";
	...
	else
	{
		...
		{
			echo "Duplicate entry detected. (Check BN Number: <b>". $data->rc_numbers[$i]."</b>, or the Business Name: <b>".$data->companies[$i]."</b> ) and make the necessary correction(s).";
			unset($data);
			return;
		}
	}
...
for ($i = 0; $i < $count; $i++){
	$sql = "INSERT INTO tbl_Business_Names (BN_Number, Business_Name, Company_Type, BN_Batch, BN_Cert_Gen_Date, ";
	$sql.= "BN_Cert_Despatch_Date, BN_Despatch_Status, BN_Cert_Manifest_Prepared_By, State_Code) VALUES ";
	$sql.="(". $data->rc_numbers[$i]."";
	$sql.=",'".$data->companies[$i]."','BN', '".$data->cert_batch[$i]."',convert(varchar(10), '".$data->cert_date[$i]."', 102), ";
	$sql.="GETDATE(),'S-T-MR','$user', '$scode')";
	$result = sqlsrv_query($conn, $sql);
	if($result === false) {
...
$bc = $data->cert_batch[0]; //E.g. FCT-BN-15072020005344-PRE-B-1
$substring ='-';
...
$event = "New Pre Incorporation Manifest Business Names -> Mail Room";
//Insert Into DB
...
sqlsrv_close( $conn );
echo "Certificates Successfully Forwarded to Mail Room. <br />Batch Code: <strong>$bc</strong><br />";
